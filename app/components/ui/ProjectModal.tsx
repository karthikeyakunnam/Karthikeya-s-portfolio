...
export default function ProjectModal({ project, onClose }: ProjectModalProps) {
  ...
  const renderLangGraph = () => {
    const nodes = [
      { id: "start", label: "START", x: 60, y: 150, desc: "Triggers on GitHub Issue Hook", tools: [] },
      { id: "planner", label: "Planner", x: 160, y: 150, desc: "Decomposes complex issues into sub-tasks", tools: ["State Manager", "Task Parser"] },
      { id: "research", label: "Research", x: 280, y: 80, desc: "Analyzes code context & semantic logs", tools: ["Pinecone RAG", "Tavily Search"] },
      { id: "critic", label: "Critic", x: 280, y: 220, desc: "Evaluates proposed code changes for safety", tools: ["LLM Judge", "Schema Validation"] },
      { id: "executor", label: "Executor", x: 400, y: 150, desc: "Applies code patches in dynamic sandbox", tools: ["Docker Exec", "FastAPI Sandbox"] },
      { id: "verifier", label: "Verifier", x: 400, y: 280, desc: "Runs unit tests and codebase validation", tools: ["Pytest Runner", "LangSmith Traces"] },
    ];
    ...
    return (
      <div className="relative w-full overflow-hidden bg-black/40 rounded-xl border border-white/5 p-4 flex flex-col items-center">
        <h4 className="text-xs font-mono text-white/40 mb-4 self-start">▸ INTERACTIVE LANGGRAPH WORKFLOW (HOVER TO EXPLORE)</h4>
            ...
            return (
              ...
              >
                ...
                <circle
                  r="14"
                  fill="#0B0F19"
                  stroke={isHovered ? "#00E5FF" : node.id === "start" ? "#FF6B6B" : "rgba(255,255,255,0.3)"}
          ...
          {hoveredNode ? (
            <div>
              <p className="text-primary font-bold mb-1">
                ▸ {nodes.find(n => n.id === hoveredNode)!.label.toUpperCase()} NODE
              </p>
              <p className="text-white/90 mb-1.5">{nodes.find(n => n.id === hoveredNode)!.desc}</p>
              {nodes.find(n => n.id === hoveredNode)!.tools.length > 0 && (
                <p className="text-white/40">
                  Tool Stack: <span className="text-accent">{nodes.find(n => n.id === hoveredNode)!.tools.join(", ")}</span>
                </p>
              )}
            </div>
          ) : (
            <p className="text-white/30 text-center py-2">Hover over graph nodes to audit agent operations</p>
          )}
        </div>
      </div>
    );
  ...
  const renderMultiAgent = () => {
    const nodes = [
      { id: "orchestrator", label: "Orchestrator", x: 250, y: 150, isHub: true, desc: "Manager LLM coordinating task distribution and plans", tools: ["State DAG Planner", "Re-planning Engine"] },
      { id: "user", label: "User Proxy", x: 90, y: 70, desc: "Ingests enterprise triggers & requirements", tools: ["Slack API", "JSON Schema Loader"] },
      { id: "planning", label: "Planning Agent", x: 410, y: 70, desc: "Decomposes instructions into sub-plans", tools: ["Few-Shot Prompts", "Graph Parser"] },
      { id: "coding", label: "Coding Worker", x: 90, y: 230, desc: "Generates custom business execution scripts", tools: ["Python Sandbox", "LLaMA-3-Coder"] },
      { id: "research", label: "Research Agent", x: 410, y: 230, desc: "Queries unstructured docs and vector stores", tools: ["Tavily API", "Redis Semantic Store"] },
      { id: "database", label: "DB Worker", x: 250, y: 270, desc: "Fetches schemas & generates optimized SQL", tools: ["Postgres API", "Explain SQL Analyzer"] },
    ];

    return (
      <div className="relative w-full overflow-hidden bg-black/40 rounded-xl border border-white/5 p-4 flex flex-col items-center">
        <h4 className="text-xs font-mono text-white/40 mb-4 self-start">▸ INTERACTIVE ORCHESTRATION GRAPH (HOVER TO EXPLORE)</h4>
        ...
        <svg viewBox="0 0 500 350" className="w-full max-w-lg h-auto select-none">
          ...
          {nodes.filter(n => !n.isHub).map((node, idx) => {
            const isActive = hoveredNode === "orchestrator" || hoveredNode === node.id;
            return (
              <g key={`spoke-${idx}`}>
                <line
                  x1="250"
          ...
          {nodes.map((node) => {
            ...
            return (
              ...
              >
                {/* Node Ring */}
                <circle
                  r={node.isHub ? (isHovered ? "28" : "24") : (isHovered ? "20" : "15")}
                  fill="#0B0F19"
                  stroke={isHovered ? "#7C3AED" : node.isHub ? "#7C3AED" : "rgba(255,255,255,0.2)"}
                ...
                >
                  {node.isHub ? "MANAGER" : node.label.split(" ")[0].toUpperCase()}
                </text>
                ...
                {!node.isHub && (
                  <text
                    ...
                    className={`font-mono text-[8px] fill-current transition-all duration-300 ${
                      isHovered ? "text-primary text-[9px] font-bold" : "text-white/40"
                    }`}
                  >
                    {node.label}
                  </text>
        ...
        <div className="mt-4 w-full min-h-[64px] p-3 rounded-lg bg-white/[0.02] border border-white/5 font-mono text-[11px] text-white/70">
          {hoveredNode ? (
            <div>
              ...
              <p className="text-white/95 mb-1.5">{nodes.find(n => n.id === hoveredNode)!.desc}</p>
              {nodes.find(n => n.id === hoveredNode)!.tools.length > 0 && (
                <p className="text-white/40">
                  Capabilities/Tools: <span className="text-accent">{nodes.find(n => n.id === hoveredNode)!.tools.join(", ")}</span>
                </p>
              )}
            </div>
          ) : (
            <p className="text-white/30 text-center py-2">Hover over manager or workers to inspect tasks and agent capabilities</p>
          )}
        </div>
      </div>
    );
  ...
  const renderRAGPipeline = () => {
    const nodes = [
      { id: "query", label: "User Query", x: 45, y: 150, desc: "User inputs semantic query or search phrase" },
      { id: "router", label: "Router & Guard", x: 120, y: 150, desc: "Evaluates query for prompt injections via LlamaGuard" },
      { id: "dense", label: "Dense Retriever", x: 215, y: 80, desc: "Pinecone semantic search on raw text chunks" },
      { id: "sparse", label: "Sparse Retriever", x: 215, y: 220, desc: "BM25 keyword search on catalog index" },
      { id: "rerank", label: "Reranker", x: 310, y: 150, desc: "Cohere cross-encoder evaluates chunk relevance scores" },
      { id: "llm", label: "LLM Gen", x: 395, y: 150, desc: "LLM synthesis with retrieved text citation indexes" },
      { id: "output", label: "Output Validation", x: 470, y: 150, desc: "Self-correcting validation runs outputs against source facts" },
    ];
    ...
    return (
      <div className="relative w-full overflow-hidden bg-black/40 rounded-xl border border-white/5 p-4 flex flex-col items-center">
        <h4 className="text-xs font-mono text-white/40 mb-4 self-start">▸ INTERACTIVE HYBRID RAG ARCHITECTURE (HOVER TO EXPLORE)</h4>

        <svg viewBox="0 0 520 320" className="w-full max-w-xl h-auto select-none">
          <defs>
          ...
          {steps.map((step, idx) => {
            ...
            const isActive = hoveredNode === step.from || hoveredNode === step.to;
          ...
          {hoveredNode ? (
            <div>
              <p className="text-accent font-bold mb-1">
                ▸ {nodes.find(n => n.id === hoveredNode)!.label.toUpperCase()} PIPELINE STEP
              </p>
              <p className="text-white/95">{nodes.find(n => n.id === hoveredNode)!.desc}</p>
            </div>
          ) : (
            <p className="text-white/30 text-center py-2">Hover over pipeline components to inspect hybrid retrieval dataflow</p>
          )}
        </div>
      </div>
    );
      ...
      >
        ...
        <div className="px-6 py-5 md:px-8 border-b border-white/5 bg-[#0B0F19]/80 backdrop-blur-md z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-white/40 text-xs md:text-sm font-mono">{project.subtitle}</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-7 space-y-8">
              ...
              <div className="p-4 bg-white/[0.01] border-l-2 rounded-r-lg border-primary/50">
                <h4 className="text-[10px] font-mono text-primary/60 uppercase tracking-widest mb-1.5">▸ ATS-FRIENDLY SUMMARY</h4>
              ...
              <div className="space-y-2">
                <h4 className="text-[10px] font-mono text-white/40 uppercase tracking-widest">▸ BUSINESS PROBLEM & GAP</h4>
              ...
              <div className="space-y-2">
                <h4 className="text-[10px] font-mono text-white/40 uppercase tracking-widest">▸ SOLUTION ARCHITECTURE</h4>
                <p className="text-white/70 text-sm leading-relaxed">
              ...
              <div className="space-y-4">
                <h4 className="text-[10px] font-mono text-white/40 uppercase tracking-widest">▸ DEEP ENGINEERING CHALLENGES</h4>
                <div className="space-y-4">
                  {project.challenges.map((challenge, idx) => (
                    <div key={idx} className="p-4 bg-white/[0.02] border border-white/5 rounded-lg space-y-2">
                      <h5 className="text-xs font-bold text-white font-mono flex items-center gap-2">
                        <span className="text-danger">⚠</span> CHALLENGE {idx + 1}: {challenge.title}
                      </h5>
                      <p className="text-[12px] text-white/50 leading-relaxed font-sans">
                        <strong className="text-white/60">Problem:</strong> {challenge.challenge}
                      </p>
                      <p className="text-[12px] text-white/70 leading-relaxed font-sans">
                        <strong className="text-accent/90">Mitigation & Solution:</strong> {challenge.solution}
                      </p>
                    </div>
                  ))}
                </div>
              ...
              {/* Key Learnings */}
              <div className="space-y-2">
                <h4 className="text-[10px] font-mono text-white/40 uppercase tracking-widest">▸ PRODUCTION TAKEAWAYS & SYSTEM LEARNINGS</h4>
                <p className="text-white/70 text-sm leading-relaxed">
                  {project.learnings}
                </p>
              </div>
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="p-5 bg-white/[0.02] border border-white/10 rounded-xl space-y-4 shadow-xl">
                <h4 className="text-[10px] font-mono text-white/40 uppercase tracking-widest">▸ KEY PERFORMANCE METRICS</h4>
                <div className="grid grid-cols-2 gap-3">
                  {Object.entries(project.metrics).map(([key, value]) => (
                    <div key={key} className="p-3 bg-black/40 border border-white/5 rounded-lg text-center flex flex-col justify-center">
                      <span className="text-xl font-bold font-mono" style={{ color: project.color }}>
                        {value}
                      </span>
              ...
              <div className="p-5 bg-white/[0.02] border border-white/10 rounded-xl space-y-3">
                <h4 className="text-[10px] font-mono text-white/40 uppercase tracking-widest">▸ RECRUITER HIGHLIGHT INDEX</h4>
                <ul className="space-y-2">
                  {project.recruiterHighlights.map((hl, idx) => (
                    <li key={idx} className="flex items-start gap-2.5 text-xs text-white/75 font-mono">
              ...
              <div className="p-5 bg-white/[0.02] border border-white/10 rounded-xl space-y-3">
                <h4 className="text-[10px] font-mono text-white/40 uppercase tracking-widest">▸ COMPLETE SYSTEM TECH STACK</h4>
              ...
              <div className="p-5 bg-white/[0.02] border border-white/10 rounded-xl space-y-3">
                <h4 className="text-[10px] font-mono text-white/40 uppercase tracking-widest">▸ SYSTEM IMPACT ACHIEVEMENTS</h4>
                <ul className="space-y-2.5">
                  {project.achievements.map((ach, idx) => (
                    <li key={idx} className="flex items-start gap-2 text-[11px] text-white/60 leading-relaxed font-sans">
                      <span className="text-[9px] mt-0.5" style={{ color: project.color }}>◈</span>
                      <span>{ach}</span>
                    </li>
                  ))}
                </ul>
